...
<template>
  <div>
    <class-header isback='yes' title="学生请假" backName="老师事务"></class-header>
    <div class="container">
      <div class="affairs-index">
        <!-- 学生请假 -->
        <div class="class-mseeage stu-leave list-change-time" v-for="(leave, index) in leavelist" :key="index">
          <div class="item mint-cell-change bg-white border-radius-10 border-grey-d" @click.stop="leaveView(leave.id,leave.leaveReplyStatus,leave.classId,leave.leaderId)">
            ...
            <mt-cell :title="leave.stuName" :label="leave.submitTime">
...
export default {
  ...
  methods: {
    queryLeaveList: function () {
      var that = this
      that.$http.post('/api/leave/viewList', {
    ...
    queryLeaderDetails: function (classId) {
      this.$http.post('/api/classical/queryDetail', {
        ...
        this.className = this.leaderDetail.clssName
      })
    },
    leaveView: function (leaveId, leaveReplyStatus, classId, leaderId) {
      let that = this
      if (leaveReplyStatus === 0 && that.user.currRole === 1) {
        that.$http.post('/api/userinfo/queryMaterial', {
          roleId: this.user.currRole
        }).then(res => {
          that.infos = res.data
          for (let i = 0; i < that.infos.length; i++) {
            let info = that.infos[i]
            if (leaderId === info.headMasterId && info.headMasterId === info.teacherId && classId === info.classId) {
              that.checkflag = true
              break
            }
          }
          if (that.checkflag === true) {
            that.$router.push({ name: '请假批复', query: { leaveId: leaveId } })
          } else {
            that.$router.push({ name: '请假详情', query: { leaveId: leaveId } })
          }
        })
      } else {
        that.$router.push({ name: '请假详情', query: { leaveId: leaveId } })
